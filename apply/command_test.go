...
func Test_Command_NewServiceError(t *testing.T) {
	err := command(configuration.Environment{}, false, func(configuration.Environment) (migration.Service, error) {
		return nil, errors.New("new service error")
	}, func(string, uint64) (*migration.Migration, error) {
		return nil, nil
	}, func([]uint64, []uint64) ([]uint64, error) {
		return nil, nil
	}, func([]string, driver.Tx) (error, error) {
		return nil, nil
	})

	if err == nil {
		t.Error("didn't failed on new service error")
		return
	}

	if err.Error() != "unable to initialize migration service: new service error" {
